class R {
  constructor(e, n, { tabInsertsSuggestions: i, firstOptionSelectionMode: s, scrollIntoViewOptions: a } = {}) {
    this.input = e, this.list = n, this.tabInsertsSuggestions = i ?? !0, this.firstOptionSelectionMode = s ?? "none", this.scrollIntoViewOptions = a ?? { block: "nearest", inline: "nearest" }, this.isComposing = !1, n.id || (n.id = `combobox-${Math.random().toString().slice(2, 6)}`), this.ctrlBindings = !!navigator.userAgent.match(/Macintosh/), this.keyboardEventHandler = (c) => G(c, this), this.compositionEventHandler = (c) => V(c, this), this.inputHandler = this.clearSelection.bind(this), e.setAttribute("role", "combobox"), e.setAttribute("aria-controls", n.id), e.setAttribute("aria-expanded", "false"), e.setAttribute("aria-autocomplete", "list"), e.setAttribute("aria-haspopup", "listbox");
  }
  destroy() {
  ...
  navigate(e = 1) {
    ...
    if (c)
      for (const h of i)
        h.removeAttribute("data-combobox-option-default"), c === h ? (this.input.setAttribute("aria-activedescendant", c.id), c.setAttribute("aria-selected", "true"), W(c), c.scrollIntoView(this.scrollIntoViewOptions)) : h.removeAttribute("aria-selected");
  }
  clearSelection() {
...
var H = { MODULE_VERSION: "3.1.1", COMPRESSED_ORG: "GYJwhgtkA", ORG_ID: "fram" };
const Pe = 300, S = String.raw, A = H.MODULE_VERSION, k = H.COMPRESSED_ORG;
function Fe(t) {
...
function qe(t, e, n) {
  const i = t.action, s = j(new FormData(t), "pw-assistant"), a = Ye({ from: e, to: n }, s), c = new URLSearchParams(a);
