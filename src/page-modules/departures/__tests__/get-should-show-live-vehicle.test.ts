...
import { getShouldShowLiveVehicle } from '../details/utils';
...
const estimatedCallsWhichDepartInGivenMinutes = (
  ...
  return [{ aimedDepartureTime: date.toISOString() }];
};

describe('getShouldShowLiveVehicle', () => {
  it('returns true if less than 10 minutes until departure time', () => {
    expect(
  ...
  it('returns true if departure time has passed', () => {
    expect(
