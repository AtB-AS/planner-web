...
function createOutput(
  ...
  function venueIconLocation() {
    const img = el('img');
    (img as HTMLImageElement).src = `${URL_BASE}assets/mono/status/Spinner.svg`;
    img.role = 'img';
  ...
  class SwapButton extends HTMLElement {
    ...
    connectedCallback() {
      ...
      button.addEventListener('click', async () => {
        ...
        const searchInput1 = searchInputs.item(0) as HTMLInputElement;
        const searchInput2 = searchInputs.item(1) as HTMLInputElement;

        if (searchInput1 == null && searchInput2 == null) {
          return;
        }

        const inputOneId = searchInput1.getAttribute('data-feature-id');

        const savedInput = searchInput1.value;
        searchInput1.value = searchInput2.value;
        searchInput2.value = savedInput;
...
async function getGeolocation(
  ...
  const img = document.querySelector('.itemLocation img');
  const imgPrevSrc = (img as HTMLImageElement).src;
  console.log(img);
  if (img) {
    (img as HTMLImageElement).src = `${urlBase}assets/mono/status/Spinner.svg`;
  }
  console.log(img);
  ...
  return new Promise(function (resolve, reject) {
    ...
    (img as HTMLImageElement).src = imgPrevSrc;
  });
}
